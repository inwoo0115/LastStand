...
void AMapWFCVisualizer::BeginPlay()
{
	Super::BeginPlay();
...
void AMapWFCVisualizer::BuildVisualization()
{
	...
	if (!Subsystem)
	{
		UE_LOG(LogTemp, Warning, TEXT("AMapWFCVisualizer::BuildVisualization - MapGeneratorSubsystem을 찾을 수 없습니다."));
		return;
	}

	// 셀 크기(수직 공용)를 읽기 위한 WFC 행 조회
	const FMapWFCData* WFCData = Subsystem->FindWFCData(MapName);
	FMapWFCData DefaultWFC;
	if (!WFCData)
	{
		UE_LOG(LogTemp, Warning, TEXT("AMapWFCVisualizer::BuildVisualization - MapWFCData 행 '%s'을(를) 찾을 수 없습니다. 기본 파라미터를 사용합니다."), *MapName.ToString());
		WFCData = &DefaultWFC;
	}
	...
	const FVector CellSize = WFCData->CellSize;
	const int32 W = Tiles.Width;
	const int32 H = Tiles.Height;
	const int32 Area = W * H;

	// 폴백 메쉬 (타일에 메쉬 미지정 시)
	UStaticMesh* FallbackMesh = WFCData->DebugMesh.LoadSynchronous();
	if (!FallbackMesh)
	{
	...
	UE_LOG(LogTemp, Log, TEXT("AMapWFCVisualizer::BuildVisualization - 타일 인스턴스 %d개 배치 (%d x %d x %d)."),
